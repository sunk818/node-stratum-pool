...
function DaemonInterface(daemons, logger){
    ...
    function performHTTPSRequest(jsonData, iProcessJson, callback) 
    {
        ...
        try
        {
            if (iProcessJson == 0)
            {
                console.log('writing %s output %s', url, data);
        ...
        catch(e)
        {
            if (data.indexOf(':-nan') !== -1){
                ...
                logger('error', 'Could not parse rpc data from daemon instance ' + jsonData + '\nReponse Data: ' + data);
