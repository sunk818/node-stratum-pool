...
var BlockTemplate = module.exports = function BlockTemplate(jobId, rpcData, poolAddressScript, extraNoncePlaceholder, reward, txMessages, recipients) {
    ...
    this.target = rpcData.target ?
        ...
        util.bignumFromBitsHex(rpcData.bits ? rpcData.bits : "00");
    ...
    this.getJobParams = function () {
        if (!this.jobParams) {
            ...
            // var nTimeOffset = parseInt(this.jobId, 16) % 16;
            var nTimeOffset = 0;
            // Verify each miner has a distinct merkleroot:  console.log('job %s merkleroot %s', this.jobId, this.rpcData.merkleroot);  (PASS)
            ...
            this.jobParams = [
                ...
                this.rpcData.bits,
                util.packUInt32BE(this.rpcData.curtime + nTimeOffset).toString('hex'),
                true
            ];
