...
var JobManager = module.exports = function JobManager(options) {
    ...
    this.processGBT = function (rpcData) {
        ...
        if (!isNewBlock) {
            ...
            if (!containsRPCData) {
                ...
                var fHexLenChanged = sHex.length != this.currentJob.rpcData.hex.length;
                var nElapsed = nTime - parseInt(this.currentJob.rpcData.curTime);
                // Not a new block:
                if (myJobHeight == rpcData.height && !fHexLenChanged && nElapsed < 30)
                    return false;
            }
        }
    ...
    this.processShare = function (jobId, previousDifficulty, difficulty, extraNonce1, extraNonce2, nTime, nonce, ipAddress,
     port, workerName, pobhhash, blockHex, x11hash, sRecipient, nABNPassed, sPoolAddress, hasabn, rpcBlockHash) {
        ...
        // var nHex = submitTime.toString('hex') + nTime.toString('hex');
