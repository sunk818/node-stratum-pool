...
var JobManager = module.exports = function JobManager(options) {
    ...
    this.processShare = function (jobId, previousDifficulty, difficulty, extraNonce1, extraNonce2, nTime, nonce, ipAddress,
     port, workerName, pobhhash, blockHex, x11hash, sRecipient, nABNPassed, sPoolAddress, hasabn, rpcBlockHash) {
        ...
        if (nTimeInt < submitTime - (60 * 7) || nTimeInt > submitTime + 7200 || nTimeInt == 0) {
            if (true) {
                console.log('ntime out of range nTime %d, rpctime %d, blockhex %s', nTimeInt, job.rpcData.curtime, blockHex);
        ...
        if (!job.registerSubmit(pobhhash, rpcBlockHash, job.rpcData.curtime, nonce)) {
