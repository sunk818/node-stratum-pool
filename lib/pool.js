    ...
    function SetupJobManager() {
        ...
        _this.jobManager.on('newBlock', function (blockTemplate) {
            ...
            var emitShare = function () {
                ...
                var sqx = 'action=hincrbyfloat&pool=' + shareData.poolOptionsAddress;
                if (isValidShare) {
                    sqx += '&validShares=' + shareData.difficulty;
                }
                else {
                    sqx += '&invalidShares=1';
                }
                ...
                sqx += '&hashratedata1=' + hashrateData1;
                sqx += '&hashratedata2=' + hashrateData2;
                sqx += '&height=' + shareData.height;
                if (isValidBlock)
                      sqx += '&solvedBlock=' + shareData.height + '&txHash=' + shareData.txHash;

                var fEnableDataExport = 1;

                if (fEnableDataExport == 1) {
                    exportShareData(sqx, function (result, fgood) {
                        if (!fgood) {
                            emitErrorLog('exportShareData call failed for daemon instance with error ' + sqx);
                            callback('error');
                        }
                    }, true);
                }
